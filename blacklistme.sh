...
if ! [[ $1 =~ ^((1?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])\.){3}(1?[0-9]?[0-9]|2[0-4][0-9]|25[0-5])((/[0-2]?[0-9])|(/3[0-2]))?$ ]];then exit;fi #  echo "This script requires a valid IP address besides the script name: $1" >> some file of your c$
...
if [ $(($(/sbin/iptables -wnL s_whitelist|grep --line-buffered -cwm 1 "$1"))) -eq 0 ] && [ $(($(/sbin/iptables -wnL s_blacklist|grep --line-buffered -cwm 1 "$1"))) -eq 0 ] && [ "$2x" != "-rx" ] && [ "$3x" != "-rx" ]; then
      ...
      if [ $(tail -n-15 /var/log/knockd.log| stdbuf -o0 grep -cwm 1 "$1: openAll: Stage") -eq 0 ]; then
          ...
          echo "if [ \$(/sbin/iptables -wL INPUT -n|grep --line-buffered -cwm 1 $1) -eq 0 ] && [ \$(/sbin/iptables -wL s_blacklist -n|grep --line-buffered -cwm 1 $1) -eq 0 ]; then /sbin/iptables -wI s_blacklist 1 -i eth0 -s $1 -m comment$
      fi
else
     if [ "$2x" == "-rx" ] || [ "$3x" == "-rx" ];then
         until [ $(/sbin/iptables -wnL s_blacklist|grep  -cwm 1 "$1") == "0" ] && [ $(/sbin/iptables -wnL d_blacklist|grep  -cwm 1 "$1") == "0" ];do
                 cmdline0="[[ \$(/sbin/iptables -wnL s_blacklist --line-numbers|grep -wm 1 $1|/usr/bin/awk '{print \$1}') -gt 0 ]] && /sbin/iptables -wD s_blacklist \$(/sbin/iptables -wnL s_blacklist --line-numbers|grep -wm 1 $1|/usr/bin$
                 cmdline0+="[[ \$(/sbin/iptables -wnL d_blacklist --line-numbers|grep -wm 1 $1|/usr/bin/awk '{print \$1}') -gt 0 ]] && /sbin/iptables -wD d_blacklist \$(/sbin/iptables -wnL d_blacklist --line-numbers|grep -wm 1 $1|/usr/bi$
                 until eval "$cmdline0";do sleep 1;done; echo "$cmdline0" >> /home/homeowner/buildiptables.sh
                 if [ $(/sbin/iptables -wnvL INPUT|/usr/bin/stdbuf -o0 grep -m 1 -c ESTABLISHED) -gt 0 ]; then /etc/init.d/netfilter-persistent save; fi
         done
     fi
fi
