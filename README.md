...
INTENDED AUDIENCE
 ...
 -- You are willing to learn a little about and install one application – iptables.  You simply cannot get an alternative to iptables as basis for any Linux firewalling solution.  It works very satisfactorily.  Just don't force too large of ruleset on it.  The ruleset size limitation depends in large measure on the amount of RAM your system avails to iptables.  I strongly recommend using a dedicated firewall box with a generous amount of RAM to maximize this size so you can collect a blacklist of probers.  If you, instead of using a separate box, run your surveillance system on the same box as this firewall solution, I recommend you be very knowledgeable of its blacklist memory useage.   If memory is scarce, consider using manual IP address consolidation techniques in the iptables ruleset for the blacklist, or script such consolidation
...
Still TODO:  – Do a better job on this documentation.  – Develop an installation script to reduce the skill level requirement of installers.  – Develop IP address consolidation for iptables rules.  At the rate of my blacklist formation, without address consolidation I estimate that my blacklist would grow to 150,000 entries in 2-3 years.  Even as robust as iptables is, I don't know that it was designed for that large of rulesets.  Integrate the services of ipset and geoip if beneficial.  – Add IPv6 functionality.  – Make code look better
