...
INTENDED AUDIENCE
 ...
 -- Plus anyone who doesn't even care about the firewalling aspect, but just wants the ability to email commands to their Linux-based automation or other purpose server.  Examine the file "email_fetch_parse" to see the default set of keywords and what actions each keyword will perform.  Some examples from that file are: "allow (IP address)" to whitelist a specific IP address, "end blacklist (IP address)" to remove from blacklist a specific IP address, "log" to insert an iptables rule that starts logging every IP packet that makes it down in the iptables INPUT chain ruleset to the rule that sends it to s_privateIPs chain, "undo logging" to undo that high level of logging, "report" to have the tail of /var/log/kern.log emailed to you, and a few more.  Make up more for your own use or for automation via usb (echo someword > /dev/[automation's usb channel]).
